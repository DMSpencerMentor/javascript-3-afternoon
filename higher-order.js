...
//Code Here
let evenNumbers = mixedNumbers.filter( (num, ind, arr) => {
  return num % 2 === 0
})
...
//Code Here
let postTaxPrices = prices.map( (num, ind, arr) => {
  return num *= 1.07
});
...
//Code Here
let totalPopulation = populations.reduce( (currTotal, element, ind, arr) => {
  return currTotal += element
})
...
//Code Here
let myStrongest = monstersInYourPocket.filter((item, ind, arr) => {
  return item.CP >= 200
})
...
let ordersTotal = orders.map(order => {
  let totalPrice = order.price + (order.price * order.tax)
})
